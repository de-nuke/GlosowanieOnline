        ...
        if get_user(request.headers['token']) != None:
            ...
            if data_read == True:
                if 'content' in data and 'title' in data:
                    ...
                    users[user]['notes'][i] = {'id': i, 'title': data['title'], 'content': data['content'],
                                               'category': category, 'tag': tag}
                    response_data = {"message": "OK", "id": i, "title": data["title"], "content": data["content"],
                                     "category": category, "tag": tag}
                    status = 200
                else:
                    response_data = {'error': 'Dane wprowadzone w niepoprawnym formacie'}
    ...
    if 'token' in request.headers:
        if get_user(request.headers['token']) != None:
            ...
            if id in users[user]['notes']:
                response_data = {"message": "OK", "id": id, "title": users[user]['notes'][id]["title"],
                                 "content": users[user]['notes'][id]["content"],
                                 "category": users[user]['notes'][id]["category"],
                                 "tag": users[user]['notes'][id]["tag"]}
                status = 200
            else:
